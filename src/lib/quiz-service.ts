...
async function ensureDataFile(): Promise<void> {
  ...
  const fileExists = await fs.access(QUIZ_FILE).then(() => true).catch(() => false)
  if (!fileExists) {
    await fs.writeFile(QUIZ_FILE, JSON.stringify({ metadata: {}, questions: [] }, null, 2), 'utf-8')
  }
}
...
// ── Public: Update quiz ──
export async function updateQuiz(id: string, params: {
  title?: string
  description?: string
  category?: string
