...
import { QuizFormState, QuizQuestion, QuizData } from '@/types/quiz'
...
export async function createQuizAction(
  ...
  try {
    ...
    const defaultTime = defaultTimeStr ? parseInt(defaultTimeStr, 10) : DEFAULT_TIME

    if (!title) {
      return { success: false, error: 'Tiêu đề quiz là bắt buộc.', field: 'title' }
    }

    const result = await createQuiz({ title, description, category, defaultTime })

    if (!result.success) {
      return { success: false, error: 'Không thể tạo quiz. Vui lòng thử lại.' }
    }
  ...
  try {
    ...
    if (questionsJson) {
      try {
        ...
        return { success: false, error: 'Dữ liệu câu hỏi không hợp lệ.', field: 'questions' }
      }
    }

    const defaultTime = defaultTimeStr ? parseInt(defaultTimeStr, 10) : undefined
    ...
    if (!result.success) {
      return { success: false, error: result.error || 'Không thể cập nhật quiz.' }
    }
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Lỗi khi cập nhật quiz.' 
    }
  }
}
