...
export async function saveQuizAction(
  ...
  try {
    ...
    if (!questionsJson) {
      return { success: false, error: 'Danh sách câu hỏi không được để trống.', field: 'questions' }
    }
    ...
    try {
      ...
      if (!Array.isArray(questions) || questions.length === 0) {
        return { success: false, error: 'Cần ít nhất một câu hỏi.', field: 'questions' }
      }
    } catch {
...
// Create quiz metadata
    ...
    const metadata = {
      ...
      defaultTime,
      timeLimit: timeLimit ? parseInt(timeLimit as string, 10) : undefined,
    }
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Lỗi khi lưu dữ liệu. Vui lòng thử lại.' 
    }
  }
}
